...
//    }
     ...
     * @param tuplecount
     */
    public static void insertBalancingTupleCount(Timestamp time,Long tuplecount,int taskid){
        try {
            PreparedStatement preparedStatement;
            String sql = "INSERT INTO t_balancinggrouping(time,tuplecount,taskid)"
                    + " VALUES (?,?,?)";  // 插入数据的sql语句
    ...
    public static void insertPKGTupleCount(Timestamp time, Long tuplecount, int taskid){
        try {
            PreparedStatement preparedStatement;
            String sql = "INSERT INTO t_pkggrouping(time,tuplecount,taskid)"
                    + " VALUES (?,?,?)";  // 插入数据的sql语句
            preparedStatement = conn.prepareStatement(sql);    // 创建用于执行静态sql语句的Statement对象
            preparedStatement.setTimestamp(1,time);
            preparedStatement.setLong(2,tuplecount);
